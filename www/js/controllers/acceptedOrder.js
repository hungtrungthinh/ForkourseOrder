...
.controller('acceptedOrderCtrl',['$scope','$location','$interval','$ionicPopup','MediaSrv','$ionicModal','$cordovaToast','config','$http','bfgData','$state','$rootScope', function($scope,$location,$interval,$ionicPopup,MediaSrv,$ionicModal,$cordovaToast,config,$http,bfgData,$state,$rootScope) {

	var Urlcall =config.clientUrl;
	//alert("1");
	sessionStorage.removeItem('newpop');	
	sessionStorage['newpage'] = "accept";
	$scope.acceptedpage='true';
    ...
    $scope.modalprintshow = function(order_id){
        $scope.accept='';
        $scope.details='';
	...
	$scope.ord.restaurant_id		=	window.localStorage['restaurant_id'];
		...
		var promise = bfgData.accepted($scope.ord,config.clientUrl,$state);
